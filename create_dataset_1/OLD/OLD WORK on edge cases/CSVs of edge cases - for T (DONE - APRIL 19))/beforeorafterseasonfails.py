...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 10 13:54:55 2024
...
def find_in_transfermarkt(player_name, season, country_code, transfermarkt_data):
    row = transfermarkt_data[(transfermarkt_data['Name'] == player_name) & (transfermarkt_data['Season'] == int(season)-1) & (transfermarkt_data['Team 1 Code'] == country_code)]
    if not row.empty:
        #mv = row.iloc[0]['Market Value']
        player_id = row.iloc[0]['PlayerID']
        #team = row.iloc[0]['Team']
        age = row.iloc[0]['Age']
        new_mv, reason = in_season_around(player_id, season, age, transfermarkt_data)
        print(new_mv)
        print(reason)
...
def in_season_around(player_id, curr_season, age, transfermarkt_data):
    ...
    if not before_season.empty and after_season.empty:
        ...
        if before_season.iloc[0]['Market Value'] != "-":
            if age >= 30:
                return (float(before_season.iloc[0]['Market Value']) * .8), "Used Season Before * .8"
            else:
                return (float(before_season.iloc[0]['Market Value']) * 1.2), "Used Season Before * 1.2"
        else:
    ...
    if not after_season.empty and before_season.empty:
        ...
        if after_season.iloc[0]['Market Value'] != "-":
            if age <= 30:
                return (float(after_season.iloc[0]['Market Value']) * .8), "Used Season After * .8"
            else:
                return (float(after_season.iloc[0]['Market Value']) * 1.2), "Used Season After * 1.2"
...
lookup_df=pd.read_csv('/Users/tedwards/Downloads/before_or_after_season(fails).csv')
